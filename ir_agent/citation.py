...
_LABELS = re.compile(
    ...
    r"|\bQ[1-4]S?\b"
    r"|第[一二三四]季度"
    # 证券代码 —— 不是需要溯源的财务数字。实测招行首稿因「600036」被驳回，
    # 白白多花一到两轮调用，且实质是在训练模型回避提及标的本身。
    r"|\b(?:sh|sz|bj|SH|SZ|BJ)?\d{6}\b"
    # 裸年份（1990–2099）。带后缀的已由上面的期间规则覆盖，这里管
    # 「2025 年度公司…」被拆开后残留的、以及「2021 至 2025 年间」这类区间。
    r"|\b(?:19[9]\d|20\d{2})\b"
)
